...
The `AudioPlayer` component provides a flexible and customizable audio playback
interface built on top of media-chrome. It features a composable architecture
that allows you to build audio experiences with custom controls, metadata
display, and seamless integration with AI-generated audio content.
...
The `AudioPlayer` component can be used to play audio from an AI SDK Speech
Result.
...
Root MediaController component. Accepts all MediaController props except `audio`
...
The audio element that contains the media source. Accepts either a remote URL or
AI SDK Speech Result data.
