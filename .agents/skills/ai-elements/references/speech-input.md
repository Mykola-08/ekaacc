...
A button component that captures voice input and converts it to text, with
...
The component extends the shadcn/ui Button component, so all Button props are
available.
...
The component automatically detects browser capabilities and uses the best
available method:
...
- **Continuous**: Set to `true` to keep recognition active until manually
  stopped
...
When the Web Speech API is unavailable, the component falls back to recording
audio:
...
**Note**: The `onAudioRecorded` prop is required for this mode to work. Without
...
- **Listening State**: Pulsing animation with accent colors to indicate active
  listening
- **Processing State**: Loading spinner while waiting for transcription
  (MediaRecorder mode)
- **Disabled State**: Button is disabled when no API is available or required
  props are missing
...
These types are properly declared for both standard and webkit-prefixed
implementations.
