...
import { Users, Calendar, DollarSign, Activity, UserCheck, Shield, Eye } from 'lucide-react';
...
function MinimalStatCard({ title, value, change, icon: Icon }: { title: string; value: string; change: string; icon: React.ElementType }) {
  return (
      ...
      <PageContainer>
        <SurfacePanel className="flex items-center justify-center">
          <div className="text-center space-y-4">
            <div className="eka-spinner w-12 h-12 mx-auto" />
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        description="Overview of platform activity and user management"
        actions={(
          <div className="flex gap-2">
            {canImpersonate && (
              <Button variant="outline" size="default" onClick={() => setShowImpersonationDialog(true)}>
                <Eye className="w-4 h-4 mr-2" />
                Impersonate User
              </Button>
            )}
            <Button variant="outline" size="default" onClick={() => window.location.href = '/admin/create-user'}>
              <Users className="w-4 h-4 mr-2" />
              Create User
            </Button>
            <Button variant="default" size="default" onClick={() => window.location.href = '/admin/settings'}>
              Settings
            </Button>
          </div>
        )}
      ...
      <SurfacePanel className="space-y-12">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.7 }}>
          <Card className="p-6 border-muted hover:border-border transition-all duration-300">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <Button variant="outline" size="default" className="w-full" onClick={() => window.location.href = '/admin/users'}>
                <Users className="w-4 h-4 mr-2" />
                Manage Users
              </Button>
              <Button variant="outline" size="default" className="w-full" onClick={() => window.location.href = '/admin/subscriptions'}>
                <Activity className="w-4 h-4 mr-2" />
                Subscriptions
              </Button>
              <Button variant="outline" size="default" className="w-full" onClick={() => window.location.href = '/admin/payments'}>
                <DollarSign className="w-4 h-4 mr-2" />
                Payments
              </Button>
              <Button variant="outline" size="default" className="w-full" onClick={() => window.location.href = '/admin/settings'}>
        ...
        {canImpersonate && (
          <UserImpersonationDialog open={showImpersonationDialog} onOpenChange={setShowImpersonationDialog} onImpersonate={handleImpersonate} />
        )}
      </SurfacePanel>
    </PageContainer>
  );
