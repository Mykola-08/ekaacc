...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // Permission checking
  const hasPermission = (permission: { group: PermissionGroup; action: PermissionAction; conditions?: Record<string, any> } | string) => {
    ...
    // Implementation depends on how permissions are stored/checked
    // This is a simplified version
    return true
  }

  const canAccessResource = (resource: string | { resource: string; action: PermissionAction; context?: Record<string, any> }, action?: string, context?: Record<string, any>) => {
    if (!state.user) return false
    if (state.user.role.name === 'admin') return true
    return true
  }
