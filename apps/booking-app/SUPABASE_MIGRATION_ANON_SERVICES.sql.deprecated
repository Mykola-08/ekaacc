...
create table if not exists anon_services (
  ...
  original_service_id uuid references service(id) on delete set null, -- Link to original if needed
...
select id, name, description, price, duration, image_url, active, 'USD'
from service
where active = true;
