...
export async function analyzeConversationSentiment(
    ...
    const positiveWords = ['happy', 'great', 'amazing', 'wonderful', 'excited', 'love', 'good', 'better', 'best', 'thankful', 'grateful', 'awesome'];
    const negativeWords = ['sad', 'angry', 'frustrated', 'stressed', 'anxious', 'worried', 'terrible', 'awful', 'bad', 'worst', 'depressed', 'hopeless', 'overwhelmed'];
    const supportWords = ['help', 'need', 'struggling', 'lonely', 'scared', 'afraid', 'hurt', 'pain', 'crisis'];
...
const BREATHING_EXERCISES: BreathingExercise[] = [
    {
        id: 'box-breathing',
        name: 'Box Breathing',
        description: 'A calming technique used by Navy SEALs. Inhale, hold, exhale, hold – each for 4 seconds.',
        duration: 4,
    ...
    },
    {
        ...
        description: 'A relaxing breath pattern that promotes sleep and calm. Inhale for 4, hold for 7, exhale for 8.',
        duration: 5,
...
export async function generateProgressReport(
    ...
    if (bookingsCompleted > 0) {
        highlights.push(`Completed ${bookingsCompleted} wellness session${bookingsCompleted > 1 ? 's' : ''}`);
    }
...
export async function startGuidedMeditation(userId: string): Promise<MeditationSession> {
    ...
    return {
        ...
        breathingPattern,
        steps,
        calmingQuote: 'The present moment is the only moment available to us, and it is the door to all moments.'
    };
}
