...
export class TelegramBotService {
    ...
    private async handleJournal(chatId: number, text: string) {
        ...
        if (!note) {
            await this.sendMessage(chatId, "Please type your note after /journal. Example: <i>/journal I felt great today!</i>");
            return;
        }

        const supabase = await createClient();
        // Identify user via telegram_links
        const { data: link } = await supabase.from('telegram_links').select('user_id').eq('telegram_chat_id', chatId).single();
    ...
    private async handleCallback(callback: any) {
        ...
        if (data === 'journal_start') {
             await this.sendMessage(chatId, "Type <code>/journal [your note]</code> to save a quick entry.");
