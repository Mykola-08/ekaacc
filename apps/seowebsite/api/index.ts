  ...
  if (count === 0) {
    const sessionTypes = [
      ...
      },
      {
        ...
        features: ['Tractament corporal complet', 'Combinació de tècniques', 'Alleujar contractures profundes', 'Equilibri energètic'],
        category: 'Massatge',
        is_active: true
      },
      {
        ...
        features: ['Tractament personalitzat complet', 'Tècniques avançades', 'Atenció detallada', 'Màxima relaxació'],
        category: 'Massatge',
        is_active: true
      },
      {
        ...
        features: ['Equilibri energètic global', 'Detecció de desequilibris', 'Reducció de l\'estrès', 'Millora de la postura'],
        category: 'Equilibri',
        is_active: true
      }
    ];
...
app.get('/api/pricing/calculate', async (c) => {
  try {
    ...
    const supabase = getSupabase();
    const { data: service } = await supabase.from('session_types').select('*').eq('id', serviceId).single();
    ...
    if (isSaturday) {
      ...
      modifiers.push({ type: 'surcharge', label: 'Suplement dissabte', amount_cents: surcharge, percentage: 10, icon: 'weekend' });
    }
    if (isSunday) {
      const surcharge = Math.round(basePriceCents * 0.2);
      finalPriceCents += surcharge;
      modifiers.push({ type: 'surcharge', label: 'Suplement diumenge', amount_cents: surcharge, percentage: 20, icon: 'weekend' });
    }

    let isVip = false;
    if (userId) {
      const { data: profile } = await supabase.from('user_profiles').select('is_vip, vip_expires_at').eq('user_id', userId).single();
      if (profile?.is_vip) {
      ...
      if (isVip) {
        ...
        modifiers.push({ type: 'discount', label: 'Descompte VIP', amount_cents: -discount, percentage: 5, icon: 'vip' });
      }
    }
    ...
    if (occupancyRate > 0.8) {
      ...
      finalPriceCents += surcharge;
      modifiers.push({ type: 'surcharge', label: 'Alta demanda', amount_cents: surcharge, percentage: 15, icon: 'high_demand' });
    }

    if (time >= '18:00' && occupancyRate > 0.6) {
      const surcharge = Math.round(basePriceCents * 0.1);
      finalPriceCents += surcharge;
      modifiers.push({ type: 'surcharge', label: 'Última franja disponible', amount_cents: surcharge, percentage: 10, icon: 'last_slot' });
    }
