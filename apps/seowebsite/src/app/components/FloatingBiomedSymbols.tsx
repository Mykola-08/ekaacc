...
import { Dna, Atom, FlaskConical, Activity, Binary, TestTube, Microscope, HeartPulse } from 'lucide-react';
...
const icons = [Dna, Atom, FlaskConical, Activity, Binary, TestTube, Microscope, HeartPulse, BenzeneRing, MoleculeStructure];
const formulas = [
 'H₂O', 'ATP', 'NAD+', 'C₈H₁₀N₄O₂', 
 'C₆H₁₂O₆', 'O₂', 'Mg²⁺', 'Ca²⁺',
 'CH₃COOH', 'NH₃', 'Fe²⁺', 'CO₂'
 ...
 useEffect(() => {
  ...
  for (let i = 0; i < 15; i++) {
    newElements.push({
      ...
      color: Math.random() > 0.5 ? 'text-blue-500' : 'text-[#FFB405]', // blue or gold
  ...
  for (let i = 0; i < 10; i++) {
    newElements.push({
      ...
      color: Math.random() > 0.5 ? 'text-blue-600' : 'text-[#FFB405]',
  ...
  for (let i = 0; i < 5; i++) {
    newElements.push({
      ...
      color: Math.random() > 0.5 ? 'text-blue-700' : 'text-[#d49604]', // slightly darker for readability
