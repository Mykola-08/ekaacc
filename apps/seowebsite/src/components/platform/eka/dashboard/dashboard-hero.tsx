        ...
        <div className="flex flex-col md:flex-row items-start justify-between gap-4">
            <div className="flex items-center gap-4">
                <Avatar className="h-16 w-16 text-2xl hidden sm:flex">
                    <AvatarImage src={currentUser.avatarUrl || ''} alt={currentUser.displayName || currentUser.email || 'User'} />
                    <AvatarFallback>{(currentUser.displayName || currentUser.email || 'U').split(' ').map((n: string) => n[0]).join('').slice(0,2).toUpperCase()}</AvatarFallback>
                </Avatar>
                <div>
                    <h1 className="text-2xl font-bold tracking-tight">Welcome back, {(currentUser.displayName || currentUser.email || 'User').split(' ')[0]}!</h1>
