...
import { User as CustomUser } from '@/lib/platform/types'
 ...
 */
export function convertSupabaseUserToCustomUser(supabaseUser: SupabaseUser | null): CustomUser | null {
  ...
  const customUser: CustomUser = {
    ...
    displayName: userMetadata.displayName || userMetadata.name || supabaseUser.email?.split('@')[0] || '',
    ...
    // Account Status
    accountStatus: (supabaseUser.email_confirmed_at ? 'active' : 'pending') as CustomUser['accountStatus'],
...
export function convertCustomUserToMetadata(customUser: Partial<CustomUser>): Record<string, any> {
  ...
  if (customUser.emergencyContact !== undefined) metadata.emergencyContact = customUser.emergencyContact
  ...
  if (customUser.therapistVisible !== undefined) metadata.therapistVisible = customUser.therapistVisible
  if (customUser.therapistNotes !== undefined) metadata.therapistNotes = customUser.therapistNotes
  if (customUser.followUpReminders !== undefined) metadata.followUpReminders = customUser.followUpReminders
  if (customUser.therapistProfile !== undefined) metadata.therapistProfile = customUser.therapistProfile
  if (customUser.profileVisibility !== undefined) metadata.profileVisibility = customUser.profileVisibility
  if (customUser.profileCompleteness !== undefined) metadata.profileCompleteness = customUser.profileCompleteness
  if (customUser.goal !== undefined) metadata.goal = customUser.goal
  if (customUser.personalizationCompleted !== undefined) metadata.personalizationCompleted = customUser.personalizationCompleted
  if (customUser.personalization !== undefined) metadata.personalization = customUser.personalization
  if (customUser.activityData !== undefined) metadata.activityData = customUser.activityData
  if (customUser.recommendations !== undefined) metadata.recommendations = customUser.recommendations
  if (customUser.personalizedContent !== undefined) metadata.personalizedContent = customUser.personalizedContent
  if (customUser.dashboardWidgets !== undefined) metadata.dashboardWidgets = customUser.dashboardWidgets
  if (customUser.subscriptionType !== undefined) metadata.subscriptionType = customUser.subscriptionType
  ...
  if (customUser.isDonationSeeker !== undefined) metadata.isDonationSeeker = customUser.isDonationSeeker
  if (customUser.isDonationSeekerApplicationPending !== undefined) metadata.isDonationSeekerApplicationPending = customUser.isDonationSeekerApplicationPending
  ...
  if (customUser.donationSeekerApproved !== undefined) metadata.donationSeekerApproved = customUser.donationSeekerApproved
  if (customUser.donationSeekerReason !== undefined) metadata.donationSeekerReason = customUser.donationSeekerReason
  ...
  if (customUser.linkedTherapist !== undefined) metadata.linkedTherapist = customUser.linkedTherapist
  if (customUser.squareCustomerId !== undefined) metadata.squareCustomerId = customUser.squareCustomerId
  if (customUser.preferredLanguage !== undefined) metadata.preferredLanguage = customUser.preferredLanguage
