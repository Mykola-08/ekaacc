...
import { safeSupabaseQuery, safeSupabaseInsert, safeSupabaseUpdate } from '@/lib/platform/supabase/utils';
...
export class TransactionManager {
  ...
  async executeInTransaction<T>(
    ...
    try {
      ...
      if (beginError) {
        throw new AppError(`Failed to begin transaction: ${beginError.message}`, 'TRANSACTION_ERROR', 500);
      }
      ...
      try {
        ...
        if (commitError) {
          throw new AppError(`Failed to commit transaction: ${commitError.message}`, 'TRANSACTION_ERROR', 500);
        }
        ...
        logger.error(`Transaction operation failed, rolling back: ${transactionId}`, error as Error);
   ...
   */
  async deleteUserWithCascade(userId: string, context: TransactionContext): Promise<TransactionResult<void>> {
    return await this.executeInTransaction([
      ...
      async () => {
        ...
        if (error) throw new AppError(`Failed to delete user sessions: ${error.message}`, 'DELETE_ERROR', 500);
      ...
      async () => {
        ...
        if (error) throw new AppError(`Failed to delete journal entries: ${error.message}`, 'DELETE_ERROR', 500);
      ...
      async () => {
        ...
        if (error) throw new AppError(`Failed to delete goals: ${error.message}`, 'DELETE_ERROR', 500);
      ...
      async () => {
        ...
        if (error) throw new AppError(`Failed to delete notifications: ${error.message}`, 'DELETE_ERROR', 500);
      ...
      async () => {
        ...
        if (error) throw new AppError(`Failed to delete user: ${error.message}`, 'DELETE_ERROR', 500);
  ...
  async bookSessionWithValidation(
    ...
    return await this.executeInTransaction([
      ...
      async () => {
        ...
        if (error) {
          throw new AppError(`Failed to check scheduling conflicts: ${error.message}`, 'DATABASE_ERROR', 500);
        }
        
        if (existingSessions && existingSessions.length > 0) {
          throw new AppError('Therapist is not available at the selected time', 'CONFLICT_ERROR', 409);
        }
  ...
  async updateSubscriptionWithProration(
    ...
    return await this.executeInTransaction([
      ...
      async () => {
        ...
        const remaining = Math.ceil((currentEndDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
...
export class DataValidator {
  ...
  static validateBooking(bookingData: any): void {
    ...
    if (bookingData.duration && (bookingData.duration < 30 || bookingData.duration > 180)) {
      throw new AppError('Session duration must be between 30 and 180 minutes', 'VALIDATION_ERROR', 400);
    }
  }
