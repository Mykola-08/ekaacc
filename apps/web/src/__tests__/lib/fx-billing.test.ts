...
describe('fxBilling', () => {
  ...
  describe('getBalanceForClient', () => {
    it('should calculate balance correctly', async () => {
      ...
      (supabase.order as jest.Mock).mockResolvedValue({ data: mockTransactions, error: null });

      const result = await fxBilling.getBalanceForClient(clientId);

      expect(result.balance).toBe(75);
      expect(result.transactions).toEqual(mockTransactions);
      expect(supabase.from).toHaveBeenCalledWith('billing_transactions');
      expect(supabase.eq).toHaveBeenCalledWith('client_id', clientId);
    });

    it('should throw error when fetch fails', async () => {
      (supabase.order as jest.Mock).mockResolvedValue({ data: null, error: new Error('DB Error') });
