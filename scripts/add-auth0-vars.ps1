...
Write-Host ""
Write-Host "Batch Adding Auth0 Variables to Vercel" -ForegroundColor Cyan
...
foreach ($key in $variables.Keys) {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "  [OK] Added to production" -ForegroundColor Green
        } else {
            Write-Host "  [FAIL] Failed: $output" -ForegroundColor Red
        }
        
        # Add to preview
        $output = Get-Content $tempFile | vercel env add $key preview 2>&1
        if ($LASTEXITCODE -eq 0) {
            Write-Host "  [OK] Added to preview" -ForegroundColor Green
        } else {
            Write-Host "  [WARN] Preview: $output" -ForegroundColor Yellow
        }
        
        # Add to development
        $output = Get-Content $tempFile | vercel env add $key development 2>&1
        if ($LASTEXITCODE -eq 0) {
            Write-Host "  [OK] Added to development" -ForegroundColor Green
        } else {
            Write-Host "  [WARN] Development: $output" -ForegroundColor Yellow
        }
...
Write-Host ""
Write-Host "Generated AUTH0_SECRET:" -ForegroundColor Cyan
Write-Host "  $AUTH0_SECRET" -ForegroundColor Yellow
Write-Host ""
Write-Host "IMPORTANT: Save this secret!" -ForegroundColor Yellow
Write-Host ""
