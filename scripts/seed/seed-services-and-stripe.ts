...
if (!STRIPE_SECRET_KEY || !SUPABASE_URL || !SUPABASE_KEY) {
  console.error(
    '❌ Missing Env Vars: STRIPE_SECRET_KEY, SUPABASE_URL, or SUPABASE_SERVICE_ROLE_KEY'
  );
  process.exit(1);
}
...
const SERVICES_TO_SYNC = [
  ...
  },
  {
    ...
    category: 'therapy',
    variants: [{ name: 'Standard Pack', amount: 16000, currency: 'eur' }],
  },
  {
  ...
  },
  {
    ...
    category: 'review_360',
    variants: [{ name: '2h Full Assessment', amount: 15000, currency: 'eur' }],
  },
];
...
async function main() {
  ...
  for (const service of SERVICES_TO_SYNC) {
    ...
    for (const variantDef of service.variants) {
      ...
      if (!price) {
        console.log(
          `       ✨ Creating new Stripe Price (${variantDef.amount / 100} ${variantDef.currency})...`
        );
