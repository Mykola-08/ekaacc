...
async function checkRegion(region: string) {
  ...
  try {
    ...
    await client.end();
    if (err.code === 'XX000' && (err.message?.includes('Tenant or user not found') || err.message?.includes('auth failed'))) {
...
async function main() {
    ...
    for (const region of regions) {
        ...
        if (result) {
            ...
            console.log(`Update your .env files with: POSTGRES_HOST="aws-0-${result}.pooler.supabase.com"`);
            process.exit(0);
        }
    }
