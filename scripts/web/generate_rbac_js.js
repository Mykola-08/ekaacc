...
try {
  ...
  permissionSet.forEach((p) => {
    sql.push(
      `  INSERT INTO permissions (key, name, description) VALUES ('${p.key}', '${p.name}', '${p.desc}') ON CONFLICT (key) DO NOTHING;`
    );
  });
  ...
  rolePermissionList.forEach((rp) => {
    sql.push(
      `  INSERT INTO role_permissions (role, permission_key) VALUES ('${rp.role}', '${rp.key}') ON CONFLICT (role, permission_key) DO NOTHING;`
    );
  });
  ...
  console.log(
    `Success! Wrote ${permissionSet.size} permissions and ${rolePermissionList.length} assignments to ${outFilePath}`
  );
} catch (e) {
  console.error(e);
}
