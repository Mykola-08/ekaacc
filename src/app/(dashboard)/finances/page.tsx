...
export default async function FinancesPage() {
  ...
  const upcomingCents = upcomingBookings
                ...
                <CardContent className="pt-0">
                  {recentTransactions.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-12 text-center text-muted-foreground">
                      <HugeiconsIcon icon={Invoice01Icon} className="size-10 mb-3 opacity-20" />
                      <p className="text-sm font-medium">No recent activity</p>
                      <p className="text-xs mt-0.5">Your transactions will appear here.</p>
                    </div>
                  ) : (
                      ...
                      {recentTransactions.map((tx) => (
                        <li key={tx.id} className="flex items-center gap-3 py-3">
                          <div className={`flex size-8 shrink-0 items-center justify-center rounded-full ${
                            tx.amount_cents >= 0 ? 'bg-success/10 text-success' : 'bg-destructive/10 text-destructive'
                          }`}>
                            <HugeiconsIcon
                ...
                <CardContent className="pt-0">
                  {transactions.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-16 text-center text-muted-foreground">
                      ...
                      <p className="text-xs mt-0.5">Add funds to get started.</p>
                    </div>
                  ) : (
                    <ul className="divide-y divide-border/50">
                      {transactions.map((tx) => (
            ...
            <TabsContent value="billing">
              <Card>
                <CardContent className="flex flex-col items-center justify-center py-16 text-center text-muted-foreground">
                  <HugeiconsIcon icon={CreditCardIcon} className="size-10 mb-3 opacity-20" />
                  <p className="text-sm font-medium">Pay as you go</p>
                  <p className="text-xs mt-0.5 max-w-xs">
                    EKA Balance uses a wallet system. Add funds to your wallet and use them for sessions.
                  </p>
                  <AddFundsDialog>
