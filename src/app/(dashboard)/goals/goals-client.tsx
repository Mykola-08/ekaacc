...
function GoalCard({
  ...
  return (
    <Card
      className={cn(
        'rounded-[var(--radius)] transition-shadow hover:shadow-sm',
        goal.is_achieved && 'opacity-80'
      )}
    >
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex min-w-0 items-start gap-3">
            <div className="bg-primary/10 mt-0.5 shrink-0 rounded-[var(--radius)] p-2">
              {goal.is_achieved ? (
                <HugeiconsIcon icon={TaskDone02Icon} className="text-success size-4" />
              ) : (
                <HugeiconsIcon icon={Target01Icon} className="text-primary size-4" />
              )}
            </div>
            <div className="min-w-0">
              <h4 className="text-foreground truncate text-sm font-semibold">{goal.title}</h4>
              {goal.description && (
          ...
          {goal.category && (
            <span
              className={cn(
                ...
                CATEGORY_BADGE[goal.category] ?? 'bg-muted text-muted-foreground'
              )}
            >
              {goal.category}
            </span>
      ...
      <CardContent className="pb-3">
        <div className="space-y-1.5">
          <div className="flex justify-between text-xs">
            ...
            <span
              className={cn(
                ...
                goal.progress_percentage >= 100 ? 'text-success' : 'text-foreground'
              )}
            >
          ...
          <Progress
            ...
            className="h-1.5 rounded-[calc(var(--radius)*0.8)]"
          />
        </div>
      </CardContent>
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="flex items-center gap-2 text-xl font-semibold tracking-tight">
            <HugeiconsIcon icon={Target01Icon} className="text-muted-foreground size-5" />
            Goals & Milestones
          </h1>
      ...
      {goals.length === 0 ? (
        <div className="flex flex-col items-center gap-3 rounded-[var(--radius)] border border-dashed py-16 text-center">
          <div className="bg-muted rounded-[var(--radius)] p-4">
            <HugeiconsIcon icon={Moon01Icon} className="text-muted-foreground/50 size-8" />
          </div>
          <div>
          ...
          <Button
            onClick={() => setShowNewDialog(true)}
            className="mt-2 gap-2 rounded-[calc(var(--radius)*0.8)]"
          >
            <HugeiconsIcon icon={PlusSignIcon} className="size-4" />
      ...
      >
        <DialogContent className="max-w-sm rounded-[var(--radius)]">
          ...
          <div className="space-y-4">
            <p className="text-muted-foreground text-sm">{checkInGoal?.title}</p>
            <div className="space-y-1.5">
              ...
              <p className="text-muted-foreground text-center text-xs">
                Drag to update from 0 to 100
              </p>
            </div>
            <Progress value={checkInValue} className="h-2.5 rounded-[calc(var(--radius)*0.8)]" />
            {checkInHistory.length > 0 && (
              <div className="space-y-1">
                <p className="text-muted-foreground text-xs font-medium">Recent updates</p>
                <div className="space-y-1.5">
