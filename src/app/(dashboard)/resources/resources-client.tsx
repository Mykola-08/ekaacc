...
const FALLBACK: Resource[] = [
  {
    ...
    description: 'A complete guide to cognitive behavioral therapy.',
    type: 'article',
    url: null,
    is_published: true,
    created_at: '',
  ...
  },
  {
    ...
    description: '10-minute guided meditations for beginners.',
    type: 'video',
    url: null,
    is_published: true,
    created_at: '',
  ...
  },
  {
    ...
    description: 'Printable PDF to help track and analyze triggers.',
    type: 'worksheet',
    url: null,
    is_published: true,
    created_at: '',
  ...
  },
  {
    ...
    description: 'Box breathing and 4-7-8 techniques for stress.',
    type: 'exercise',
    url: null,
    is_published: true,
    created_at: '',
  ...
  },
  {
    ...
    description: 'Evidence-based practices for better sleep quality.',
    type: 'article',
    url: null,
    is_published: true,
    created_at: '',
  ...
  },
  {
    ...
    description: 'The 5-4-3-2-1 method for managing panic attacks.',
    type: 'exercise',
    url: null,
    is_published: true,
    created_at: '',
...
export function ResourcesPageClient({
  ...
  const usedTypes = [
    'all',
    ...Array.from(new Set(displayResources.map((r) => r.type).filter(Boolean))),
  ];
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <h1 className="text-foreground text-xl font-semibold tracking-tight">Resource Library</h1>
        ...
        {usedTypes.map((type) => {
          ...
          const count =
            type === 'all'
              ...
              : displayResources.filter((r) => r.type === type).length;
          return (
            <button
              key={type}
        ...
        <div className="grid gap-4 @xl/main:grid-cols-2 @5xl/main:grid-cols-3">
          {filtered.map((r) => {
            ...
            return (
              <Card
                key={r.id}
                className="group border-border/60 flex flex-col rounded-[var(--radius)] border transition-shadow hover:shadow-md"
              >
                <CardHeader className="pb-3">
                  <div className="flex items-start gap-3">
                    <div className={cn('shrink-0 rounded-[var(--radius)] p-2.5', cfg.color)}>
                      <HugeiconsIcon icon={cfg.icon} className="size-5" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-start gap-2">
                        <h3 className="text-foreground flex-1 text-sm leading-snug font-semibold">
                          {r.title}
                        </h3>
                        {isCompleted && (
                          <Badge
                  ...
                  <CardContent className="pt-0 pb-3">
                    <div className="flex flex-wrap gap-1">
                      {(r.tags ?? []).slice(0, 4).map((tag) => (
                        <span
                          key={tag}
                          className="bg-muted text-muted-foreground rounded-[calc(var(--radius)*0.8)] px-2 py-0.5 text-xs capitalize"
                        >
                          {tag}
                        </span>
                      ))}
