  ...
  return (
    <div className="flex-1 space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {forms.map((form, idx) => (
          <Card key={idx} className="rounded-[calc(var(--radius)*2.2)] border border-border/50 shadow-sm relative overflow-hidden group">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                ...
                {form.status === 'completed' ? (
                  <span className="flex items-center gap-1 text-xs font-medium text-primary bg-primary/10 px-2.5 py-1 rounded-[calc(var(--radius)*0.8)]">
                    <HugeiconsIcon icon={CheckmarkCircle01Icon} className="size-3" /> Completed
                  </span>
                ) : (
                  <span className="flex items-center gap-1 text-xs font-medium text-destructive bg-destructive/10 px-2.5 py-1 rounded-[calc(var(--radius)*0.8)]">
