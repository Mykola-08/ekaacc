    ...
    <div className="flex-1 space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {forms.map((form, idx) => (
          <Card
            key={idx}
            className="border-border/50 group relative overflow-hidden rounded-[calc(var(--radius)*2.2)] border shadow-sm"
          >
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div
                  className={`rounded-full p-2.5 ${form.status === 'completed' ? 'bg-primary/10 text-primary' : 'bg-destructive/10 text-destructive'}`}
                >
                  <HugeiconsIcon icon={File01Icon} className="size-5" />
                </div>
                {form.status === 'completed' ? (
                  <span className="text-primary bg-primary/10 flex items-center gap-1 rounded-[calc(var(--radius)*0.8)] px-2.5 py-1 text-xs font-medium">
                    <HugeiconsIcon icon={CheckmarkCircle01Icon} className="size-3" /> Completed
                  </span>
                ) : (
                  <span className="text-destructive bg-destructive/10 flex items-center gap-1 rounded-[calc(var(--radius)*0.8)] px-2.5 py-1 text-xs font-medium">
                    <HugeiconsIcon icon={Time01Icon} className="size-3" /> Pending
                  </span>
                )}
              </div>
              <CardTitle className="mt-4 text-lg">{form.title}</CardTitle>
              <CardDescription>{form.date}</CardDescription>
            </CardHeader>
            <CardContent>
              <Button
                variant={form.status === 'completed' ? 'outline' : 'default'}
                className="w-full rounded-[calc(var(--radius)*0.8)]"
              >
