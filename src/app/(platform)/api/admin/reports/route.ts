...
async function generateSystemHealthReport(dateFrom?: string, dateTo?: string) {
  let query = supabaseAdmin.from('audit_logs').select('*').like('action', '%response_time%');
      ...
      const value =
        row[
          Object.keys(row).find(
            (key) => key.toLowerCase().replace(/_/g, '') === col.toLowerCase().replace(/\s+/g, '')
          ) || ''
        ];
