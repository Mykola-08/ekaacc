...
export default function DonationsPage() {
  ...
  useEffect(() => {
    if (dataService && currentUser?.id) {
      setIsLoading(true);
      Promise.all([dataService.getDonations(currentUser.id), dataService.getAllUsers()])
  ...
  const potentialRecipients = useMemo(
    () =>
      allUsers.filter(
        (u: any) => u.id !== currentUser?.id && (u as any).user_metadata?.isDonationSeeker
      ),
          ...
          <Card>
            ...
            <CardContent className="grid gap-6">
              ...
              >
                ...
                {t('donations.page.donateButton').replace('€', '')}€{amount || '...'}
              </Button>
            </CardContent>
          </Card>
        </div>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground dark:text-muted-foreground/80">
                  {t('donations.page.totalDonated')}
                </span>
                <span className="text-xl font-bold">€{donationStats.totalDonated.toFixed(2)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground dark:text-muted-foreground/80">
                  {t('donations.page.sessionsFunded')}
                </span>
                <span className="text-xl font-bold">{donationStats.sessionsFunded}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground dark:text-muted-foreground/80">
                  {t('donations.page.livesTouched')}
                </span>
                <span className="text-xl font-bold">{donationStats.livesTouched}</span>
              </div>
            </CardContent>
          </Card>
