        ...
        <AccordionContent>
         <div className="space-y-4 pt-4">
          <div className="flex gap-2">
           ...
           />
           <Button size="sm" variant="ghost" disabled>Saved on blur</Button>
          </div>
          
          <div className="space-y-2 pl-4 border-l-2">
           {module.lessons?.map((lesson) => (
            <div key={lesson.id} className="flex justify-between items-center p-2 border rounded bg-muted/50">
             <div className="flex items-center gap-2">
              <span className="text-sm font-medium">{lesson.title}</span>
              <span className="text-xs text-muted-foreground uppercase">({lesson.content_type})</span>
             </div>
          ...
          <Button size="sm" variant="outline" className="w-full" onClick={() => handleAddLesson(module.id)}>
