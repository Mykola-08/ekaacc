...
const progressData = {
  ...
  achievements: [
    ...
    },
    {
      id: '3',
      name: 'Pain Progress',
      description: 'Reduced pain by 50%',
      earned: true,
      date: '2025-10-15',
    },
    {
      id: '4',
      name: 'Halfway There',
      description: 'Completed 50% of your session goal',
      earned: true,
      date: '2025-10-18',
    },
    {
      id: '5',
      name: 'Goal Achieved',
      description: 'Reached your wellness goal',
      earned: false,
      date: null,
    },
  ],
    ...
    <div className="space-y-8">
      ...
      <Tabs defaultValue="progress" className="w-full">
        ...
        <TabsContent value="progress" className="space-y-8">
              ...
              <CardContent>
                ...
                <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-muted dark:bg-foreground/80">
                  <div
                    className="h-full bg-blue-600 transition-all duration-300"
                    style={{
                      width: `${(progressData.sessionsCompleted / progressData.sessionGoal) * 100}%`,
                    }}
                  />
                </div>
              </CardContent>
          ...
          <Tabs defaultValue="pain" className="mt-8 w-full">
                        ...
                        <div className="flex-1">
                          <div className="h-8 w-full overflow-hidden rounded-full bg-muted dark:bg-foreground/80">
                            <div
                              className="h-full bg-blue-600 transition-all duration-300"
                              style={{
                                width: `${(entry.score / progressData.mobility.target) * 100}%`,
                              }}
                            />
                          </div>
                        </div>
                ...
                <CardContent>
                  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {progressData.achievements.map((ach) => (
                      <Card
                        key={ach.id}
                        className={ach.earned ? 'border-green-500' : 'border-muted'}
                      >
                        <CardHeader>
            ...
            <div className="space-y-8 lg:col-span-2">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    View all your session summaries and progress reports.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  {isLoadingReports && (
                  ...
                  {!isLoadingReports && sortedReports && sortedReports.length > 0 && (
                    <ul className="space-y-1">
                      {sortedReports.map((report) => (
                        <li
                          key={report.id}
                          className="hover:bg-muted/50 flex items-start gap-4 rounded-[12px] p-4 transition-colors"
                        >
                          <div className="bg-muted flex shrink-0 items-center justify-center rounded-full p-3">
                            {report.type === 'AI Summary' ? (
                          ...
                          <div className="flex-1 overflow-hidden">
                            ...
                            <p className="text-muted-foreground text-sm">
                              {report.author} -{' '}
                              {report.date
                                ? format(toDate(report.date), 'MMMM d, yyyy')
                                : 'No date'}
                            </p>
                            <p className="mt-1 text-sm break-words">{report.summary}</p>
                          </div>
                          <Button variant="outline" size="sm" className="shrink-0">
                            <ArrowUp className="h-4 w-4 -rotate-45 transform" />
                          </Button>
                        </li>
                      ))}
                    </ul>
                  ...
                  {!isLoadingReports && (!sortedReports || sortedReports.length === 0) && (
                    <div className="py-12 text-center">
                      ...
                      <p className="text-muted-foreground mt-1 text-sm">
                        Your session reports and summaries will appear here.
                      </p>
                    </div>
                  )}
                </CardContent>
