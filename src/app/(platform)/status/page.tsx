    ...
    <div className="rounded-[20px] border-none border-white/10 bg-black/40 p-6 backdrop-blur-sm transition-colors hover:border-white/20">
      <div className="mb-4 flex items-start justify-between">
        ...
        <div className="flex flex-col items-end">
          {service.uptime && (
            <span className="font-mono text-xs text-green-400">{service.uptime} uptime</span>
          )}
          {service.latency && (
            <span className="font-mono text-xs text-zinc-500">{service.latency}ms</span>
          )}
        </div>
      </div>

      <h3 className="mb-1 text-lg font-semibold text-white">{service.name}</h3>
      <p className="mb-4 h-10 text-sm text-zinc-400">{service.description}</p>
      ...
      <div className="mt-auto flex items-center justify-between">
        ...
        {service.error && (
          <span className="max-w-[150px] truncate text-xs text-red-400" title={service.error}>
            {service.error}
          </span>
        )}
      </div>
      ...
      <div className="mx-auto max-w-6xl space-y-12">
        ...
        <div className="space-y-2">
          <div className="flex items-start justify-between">
            ...
            {data?.userRole && (
              <span className="rounded border border-zinc-800 px-2 py-1 font-mono text-xs text-zinc-600">
                View: {data.userRole}
              </span>
            )}
          </div>
