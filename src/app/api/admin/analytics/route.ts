  ...
  try {
    ...
    if (metric === 'all' || metric === 'sessions') {
      ...
      const completedSessions =
        sessionStats.data?.filter((s) => s.status === 'completed').length || 0;
      const avgDuration = sessionStats.data
        ? sessionStats.data.reduce((acc, s) => acc + (s.duration || 0), 0) / (totalSessions || 1)
        : 0;
      ...
      results.sessions = {
        ...
        byTherapist:
          sessionStats.data?.reduce((acc: any, session) => {
    ...
    if (metric === 'all' || metric === 'revenue') {
      ...
      const totalRevenue =
        revenueStats.data?.reduce(
          (acc, p) => (p.status === 'succeeded' ? acc + (p.amount || 0) : acc),
          0
        ) || 0;

      const successfulPayments =
        revenueStats.data?.filter((p) => p.status === 'succeeded').length || 0;
    ...
    if (metric === 'all' || metric === 'system') {
      ...
      const errorLogs =
        systemStats.data?.filter(
          (log) => log.action?.includes('error') || log.action?.includes('failed')
        ).length || 0;

      const securityEvents =
        systemStats.data?.filter(
          (log) => log.action?.includes('security') || log.action?.includes('auth')
        ).length || 0;
