...
async function getAuditSummary(dateFrom?: string, dateTo?: string) {
  ...
  const summary = {
    ...
    errors:
      data?.filter((log) => log.action?.includes('error') || log.action?.includes('failed'))
        .length || 0,
    securityEvents:
      data?.filter((log) => log.action?.includes('security') || log.action?.includes('auth'))
        .length || 0,
...
async function analyzePermissionChanges(timeframe: string = '7d') {
  ...
  const elevatedPermissions =
    permissionChanges.data?.filter(
      (log) => log.details?.includes('admin') || log.details?.includes('superuser')
    ) || [];
...
async function analyzeDataAccess(timeframe: string = '7d') {
  ...
  const bulkAccess =
    dataAccess.data?.filter(
      (log) => log.details?.includes('bulk') || log.details?.includes('export')
    ) || [];
