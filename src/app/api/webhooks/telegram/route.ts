...
async function handleMessage(update: TelegramUpdate) {
  ...
  if (text?.startsWith('/')) {
    ...
    switch (command) {
      case '/start':
        if (args) {
          // If launched via deep link: t.me/bot?start=CODE
    ...
    // Normal conversation message! Process via AI
    await handleAiConversation(chat.id, from, text);
  }
}
...
async function handleAiConversation(chatId: number, from: any, text: string) {
  try {
    ...
    if (!userId) {
      await sendMessage(
        chatId,
        "Hi! I'm EKA AI. Please link your account first to chat with me. Use /link [code]."
      );
      return;
    }
  ...
  try {
    ...
    if (data.startsWith('services_page:')) {
      ...
      if (offset > 0) {
        navRow.push({
          text: '◀️ Previous',
          callback_data: `services_page:${Math.max(0, offset - PAGE_SIZE)}`,
        });
      }
          ...
          replyMarkup: {
            inline_keyboard: [
              [
                {
                  text: '📅 Open Booking',
                  web_app: { url: `${siteUrl()}/booking/${serviceSlug}` },
                },
              ],
            ],
          },
    ...
    if (data === 'cancel_abort') {
      ...
      if (chatId && messageId) {
        await editMessageText(
          chatId,
          messageId,
          '👍 <b>Booking kept.</b> Your appointment is still scheduled.',
          {
            replyMarkup: {
              inline_keyboard: [[{ text: '📋 My Appointments', callback_data: 'my_appointments' }]],
            },
          }
        );
      }
    ...
    if (data.startsWith('rebook:')) {
      ...
      await sendMessage(
        chatId,
        [`🔄 <b>Rebook: ${service.name}</b>`, '', 'Tap below to book a new appointment:'].join(
          '\n'
        ),
        {
          replyMarkup: {
...
async function handleStartCommand(
  chatId: number,
  from?: TelegramUpdate['message'] extends { from?: infer F } ? F : never
) {
    ...
    ].join('\n'),
    {
      replyMarkup: {
        inline_keyboard: [
          [
            {
              text: '🌐 Visit EKA Balance',
              url: process.env.NEXT_PUBLIC_SITE_URL ?? 'https://ekabalance.com',
            },
            {
              text: '📅 Book Now',
              url: `${process.env.NEXT_PUBLIC_SITE_URL ?? 'https://ekabalance.com'}/booking`,
            },
          ],
        ],
      },
...
async function handleLinkCommand(chatId: number, from: any, code: string) {
  ...
  // Look for a matching code in user_profiles metadata
  const { data: profiles } = await supabase
    .from('user_profiles')
    .select('user_id, metadata')
    // Postgres JSONB query
    .contains('metadata', { telegram_connect_code: cleanCode });
  ...
  if (!profile) {
    await sendMessage(
      chatId,
      '❌ Invalid or expired link code. Please generate a new one from your EKA dashboard.'
    );
    return;
  }
  ...
  if (meta.telegram_connect_expires && Date.now() > meta.telegram_connect_expires) {
    await sendMessage(
      chatId,
      '❌ Link code has expired. Please generate a new one from your EKA dashboard.'
    );
    return;
  }

  // Upsert the telegram_links record
  ...
  if (error) {
    ...
    await sendMessage(
      chatId,
      '❌ Error linking account. It might already be linked to another user.'
    );
    return;
  }

  // Clear code from metadata
  delete meta.telegram_connect_code;
  delete meta.telegram_connect_expires;
  await supabase.from('user_profiles').update({ metadata: meta }).eq('user_id', profile.user_id);
...
async function handleUnlinkCommand(chatId: number, from: any) {
  ...
  if (link) {
    ...
    await sendMessage(
      chatId,
      '🔓 Account unlinked. You will no longer receive notifications here.'
    );
  } else {
...
async function handleBookCommand(chatId: number, from: any, args: string) {
  try {
    ...
    if (args?.trim()) {
      ...
      await sendMessage(
        chatId,
        `❌ Service "<b>${args.trim()}</b>" not found. Use /book to see all services.`
      );
      return;
    }
    ...
    if (services.length > PAGE_SIZE) {
      serviceButtons.push([
        {
          text: `▶️ More (${services.length - PAGE_SIZE} more)`,
          callback_data: `services_page:${PAGE_SIZE}`,
        },
      ]);
    }
...
async function handleAppointmentsCommand(chatId: number, from: any) {
  ...
  try {
    ...
    buttons.push([{ text: '🌐 Open Dashboard', web_app: { url: `${siteUrl()}/dashboard` } }]);
...
async function handleHistoryCommand(chatId: number, from: any) {
  ...
  try {
    ...
    if (!bookings || bookings.length === 0) {
      await sendMessage(
        chatId,
        ['📚 <b>Booking History</b>', '', "You don't have any past bookings yet."].join('\n'),
        {
          replyMarkup: {
            inline_keyboard: [
    ...
    buttons.push([{ text: '🌐 Full History', web_app: { url: `${siteUrl()}/dashboard` } }]);
...
async function handleDetailsCommand(chatId: number, from: any, args: string) {
  ...
  try {
    ...
    if (['completed', 'cancelled'].includes(booking.status)) {
      actionButtons.push([
        {
          text: '🔄 Rebook This Service',
          callback_data: `rebook:${booking.service_id ?? booking.serviceId}`,
        },
      ]);
    }
...
async function handleMoodCommand(chatId: number, from: any, args: string) {
  ...
  if (isNaN(score) || score < 1 || score > 10) {
    await sendMessage(
      chatId,
      '⚠️ Mood score must be between 1 and 10. Example: <code>/mood 7 feeling great</code>'
    );
    return;
  }
    ...
    await sendMessage(
      ...
      [
        ...
        trend.moods.length > 1
          ? `${trendIcon} 14-day trend: <b>${trend.trend}</b> (avg: ${trend.averageScore})`
          : '',
        '',
...
async function handleResourcesCommand(chatId: number, args: string) {
  try {
    ...
    if (!filterCategory) {
      ...
      buttons.push([{ text: '📚 All Resources', callback_data: 'resources:all' }]);
    }
            ...
            [
              {
                ...
                web_app: {
                  url: `${siteUrl()}/booking/${booking.service_slug}?reschedule=${bookingId}`,
                },
              },
            ],
            [
