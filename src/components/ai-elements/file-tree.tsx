...
import { ArrowRight01Icon, FileAttachmentIcon, FolderLibraryIcon, FolderOpenIcon } from 'hugeicons-react';
...
export const FileTree = ({
  ...
  return (
    <FileTreeContext.Provider value={contextValue}>
      <div
        className={cn(
          'bg-background rounded-[calc(var(--radius)*0.8)] border font-mono text-sm',
          className
        )}
        role="tree"
        {...props}
            ...
            >
              <FileTreeIcon>
                {isExpanded ? (
                  ...
                  <FolderLibraryIcon className="size-4 text-blue-500" />
                )}
              </FileTreeIcon>
      ...
      >
        {children ?? (
          <>
            ...
            <FileTreeIcon>
              {icon ?? <FileAttachmentIcon className="text-muted-foreground size-4" />}
            </FileTreeIcon>
            <FileTreeName>{name}</FileTreeName>
          </>
        )}
