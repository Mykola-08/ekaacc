...
import { ChevronDownIconIcon, PaperclipIconIcon } from 'hugeicons-react';
...
export const QueueItem = ({ className, ...props }: QueueItemProps) => (
  <li
    className={cn(
      'group hover:bg-muted flex flex-col gap-1 rounded-[calc(var(--radius)*0.6)] px-3 py-1 text-sm transition-colors',
      className
    )}
    {...props}
  />
...
export const QueueSectionTrigger = ({
  ...
  <CollapsibleTrigger asChild>
    <button
      className={cn(
        'group bg-muted/40 text-muted-foreground hover:bg-muted flex w-full items-center justify-between rounded-[calc(var(--radius)*0.6)] px-3 py-2 text-left text-sm font-medium transition-colors',
        className
      )}
      type="button"
      {...props}
...
export const QueueSectionLabel = ({
  ...
  <span className={cn('flex items-center gap-2', className)} {...props}>
    <ChevronDownIconIcon className="size-4 transition-transform group-data-[state=closed]:-rotate-90" />
    {icon}
    <span>
      {count} {label}
    </span>
...
export const Queue = ({ className, ...props }: QueueProps) => (
  <div
    className={cn(
      'border-border bg-background flex flex-col gap-2 rounded-[var(--radius)] border px-3 pt-2 pb-2 shadow-xs',
      className
    )}
    {...props}
  />
