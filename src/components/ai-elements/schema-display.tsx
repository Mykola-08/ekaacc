...
import { ChevronRightIconIcon } from 'hugeicons-react';
  ...
  return (
    <span className={cn('font-mono text-sm', className)} {...props}>
      {children ? (
        ...
        <span dangerouslySetInnerHTML={{ __html: highlightedPath }} />
      )}
    </span>
  );
};
...
export const SchemaDisplayParameter = ({
  ...
  <div className={cn('px-4 py-3 pl-10', className)} {...props}>
    <div className="flex items-center gap-2">
      ...
      {required && (
        <Badge className="bg-red-100 text-xs text-red-700" variant="secondary">
          required
        </Badge>
      )}
    </div>
...
export const SchemaDisplay = ({
  ...
  return (
    <SchemaDisplayContext.Provider value={contextValue}>
      <div
        className={cn(
          'bg-background overflow-hidden rounded-[calc(var(--radius)*0.8)] border',
          className
        )}
        {...props}
      >
...
export const SchemaDisplayExample = ({
  ...
  <pre
    className={cn(
      'bg-muted mx-4 mb-4 overflow-auto rounded-[calc(var(--radius)*0.6)] p-4 font-mono text-sm',
      className
    )}
    {...props}
  >
