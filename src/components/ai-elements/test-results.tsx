...
import { CheckCircle2IconIcon, ChevronRightIconIcon, CircleDotIconIcon, CircleIconIcon, XCircleIconIcon } from 'hugeicons-react';
      ...
      {children ?? (
        <>
          <Badge className="gap-1 bg-green-100 text-green-700" variant="secondary">
            <CheckCircle2IconIcon className="size-3" />
            {summary.passed} passed
          </Badge>
          {summary.failed > 0 && (
            <Badge className="gap-1 bg-red-100 text-red-700" variant="secondary">
              <XCircleIconIcon className="size-3" />
              {summary.failed} failed
            </Badge>
          )}
          {summary.skipped > 0 && (
            <Badge className="gap-1 bg-yellow-100 text-yellow-700" variant="secondary">
              <CircleIconIcon className="size-3" />
...
export const TestResults = ({ summary, className, children, ...props }: TestResultsProps) => {
  ...
  return (
    <TestResultsContext.Provider value={contextValue}>
      <div
        className={cn('bg-background rounded-[calc(var(--radius)*0.8)] border', className)}
        {...props}
      >
        {children ??
          (summary && (
...
const statusIcons: Record<TestStatus, React.ReactNode> = {
  failed: <XCircleIconIcon className="size-4" />,
  passed: <CheckCircle2IconIcon className="size-4" />,
  running: <CircleDotIconIcon className="size-4 animate-pulse" />,
  skipped: <CircleIconIcon className="size-4" />,
};
...
export const TestSuite = ({ name, status, className, children, ...props }: TestSuiteProps) => {
  ...
  return (
    <TestSuiteContext.Provider value={contextValue}>
      <Collapsible className={cn('rounded-[calc(var(--radius)*0.8)] border', className)} {...props}>
        {children}
      </Collapsible>
...
export const TestSuiteName = ({ className, children, ...props }: TestSuiteNameProps) => {
  ...
  return (
    <CollapsibleTrigger
      ...
      {...props}
    >
      <ChevronRightIconIcon className="text-muted-foreground size-4 shrink-0 transition-transform group-data-[state=open]:rotate-90" />
...
export const TestError = ({ className, children, ...props }: TestErrorProps) => (
  <div className={cn('mt-2 rounded-[calc(var(--radius)*0.6)] bg-red-50 p-3', className)} {...props}>
    {children}
  </div>
);
...
export const TestErrorMessage = ({ className, children, ...props }: TestErrorMessageProps) => (
  <p className={cn('text-sm font-medium text-red-700', className)} {...props}>
    {children}
  </p>
);
...
export const TestErrorStack = ({ className, children, ...props }: TestErrorStackProps) => (
  <pre className={cn('mt-2 overflow-auto font-mono text-xs text-red-600', className)} {...props}>
    {children}
  </pre>
);
