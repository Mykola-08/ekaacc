  ...
  return (
    <div className="container max-w-5xl py-12 px-4 md:px-6 flex flex-col md:flex-row gap-8">
      ...
      <div className="w-full md:w-1/3 order-2 md:order-1 animate-in fade-in slide-in-from-right-8 duration-700">
        <div className="bg-card rounded-[36px] border border-border shadow-[0_4px_24px_rgba(0,0,0,0.02)] overflow-hidden sticky top-24">
          <div className="relative h-48 bg-muted">
            {/* Image Placeholder */}
            <div className="absolute inset-0 flex items-center justify-center text-muted-foreground/20">
              <HugeiconsIcon icon={UserGroupIcon} className="w-16 h-16" strokeWidth={1} />
            </div>
             {/* Live Viewers Badge */}
             {viewerCount > 1 && (
      ...
      <div className="w-full md:w-2/3 order-1 md:order-2">
            ...
            <Button variant="ghost" size="sm" onClick={() => step !== 'time' ? handleBack() : router.back()} className="mb-4 pl-0 hover:pl-0 text-muted-foreground hover:text-foreground">
                 <HugeiconsIcon icon={ArrowLeft01Icon} className="w-4 h-4 mr-2"/> {step === 'time' ? "Back to Service" : "Back"}
            </Button>
         ...
         <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            {step === "time" && (
                <div className="grid lg:grid-cols-2 gap-8">
                    <div className="bg-card rounded-[36px] p-6 shadow-sm border border-border">
                        <Calendar
                            mode="single"
                            selected={date}
                            onSelect={setDate}
                    ...
                    <div className="space-y-6">
                        ...
                        {loadingSlots ? (
                             <div className="flex items-center justify-center py-12 text-muted-foreground">
                                 <HugeiconsIcon icon={Loading03Icon} className="w-6 h-6 animate-spin mr-2" />
                                 Looking for slots...
                             </div>
                        ) : timeSlots.length > 0 ? (
            ...
            {step === "details" && (
                <div className="bg-card rounded-[36px] p-8 border border-border shadow-sm">
             ...
             {step === "payment" && (
                <div className="space-y-6">
                                   ...
                                   walletBalance !== null && walletBalance >= price ? (
                                       <div className="text-center space-y-4 py-4">
                                           <div className="bg-emerald-500/10 text-emerald-600 rounded-full p-4 w-16 h-16 mx-auto flex items-center justify-center">
                                               <HugeiconsIcon icon={Tick02Icon} className="w-8 h-8" strokeWidth={3} />
                                           </div>
                                           <div>
                                       ...
                                       <div className="text-center space-y-6 py-4">
                                           <div className="bg-amber-500/10 text-amber-600 rounded-full p-4 w-16 h-16 mx-auto flex items-center justify-center">
                                               <HugeiconsIcon icon={CreditCardIcon} className="w-8 h-8" strokeWidth={3} />
                                           </div>
                                           <div>
                    ...
                    <div className="flex items-start space-x-3 text-sm text-muted-foreground px-2">
                        <HugeiconsIcon icon={Tick02Icon} className="w-5 h-5 text-emerald-600 shrink-0" strokeWidth={2.5} />
                 ...
                 <div className="ml-auto">
                    {step === 'payment' ? (
                        <Button size="lg" className="px-10 h-14 rounded-full font-bold text-lg shadow-xl hover:scale-105 active:scale-95 transition-all" onClick={handleBooking} disabled={loading}>
                            {loading ? <HugeiconsIcon icon={Loading03Icon} className="w-5 h-5 animate-spin mr-2"/> : null}
                            Confirm Booking
                        </Button>
                    ) : (
