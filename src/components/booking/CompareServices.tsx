...
export function CompareServices() {
    return (
        <div className="w-full bg-card rounded-[36px] border border-border/60 shadow-none overflow-hidden animate-in slide-in-from-bottom-4 duration-700">
...
function FeatureRow({ label, tooltip, classic, premium, royal }: any) {
    ...
    return (
        <tr className="hover:bg-secondary/30 transition-all group">
            <td className="p-4 pl-8 group-hover:pl-9 transition-all text-sm font-semibold text-muted-foreground flex items-center gap-2">
                ...
                {tooltip && (
                    <TooltipProvider>
                        <Tooltip>
                            <TooltipTrigger><HugeiconsIcon icon={InformationCircleIcon} className="w-3.5 h-3.5 text-muted-foreground/50" /></TooltipTrigger>
