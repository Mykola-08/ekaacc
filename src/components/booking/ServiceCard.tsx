...
export function ServiceCard({ service, variant = 'default' }: ServiceCardProps) {
  ...
  return (
    <div className="h-full hover:scale-[1.02] transition-transform duration-500 ease-out origin-center active:scale-[0.98]">
      <Card className={cn(
        "h-full flex flex-col overflow-hidden transition-all duration-300",
        "bg-card border-border shadow-none", // Porcelain Surface
        isCompact ? "rounded-2xl" : "rounded-[36px]"
      )}>
        ...
        <CardContent className={cn("flex-grow", isCompact ? "p-5 pt-0" : "p-7 pt-0")}>
          <div className="flex flex-col gap-3 text-sm text-muted-foreground font-medium">
            <div className="flex items-center gap-3">
              <HugeiconsIcon icon={Clock01Icon} className="w-4 h-4 text-muted-foreground/60" strokeWidth={2.75} />
        ...
        <CardFooter className={cn("border-t border-border bg-secondary/30", isCompact ? "p-4" : "p-7")}>
          <Button asChild className="w-full gap-2 group rounded-full bg-primary hover:bg-primary/90 text-primary-foreground shadow-none hover:shadow-lg hover:shadow-primary/20 transition-all font-semibold active:scale-95 h-12 text-[15px]" size={isCompact ? "sm" : "lg"}>
            <Link href={`/book/${service.id}`}>
              Book Session
              <HugeiconsIcon icon={ArrowRight01Icon} className="w-4 h-4 transition-transform group-hover:translate-x-1" strokeWidth={2.5} />
            </Link>
          </Button>
        </CardFooter>
      </Card>
