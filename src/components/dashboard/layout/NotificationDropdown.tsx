  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" size="icon" className="h-10 w-10 rounded-full relative border-none bg-card hover:bg-secondary shadow-sm hover:shadow-md transition-all">
          <HugeiconsIcon icon={Notification01Icon} className="h-4 w-4 text-foreground" strokeWidth={2.5} />
          {unreadCount > 0 && (
             <span className="absolute top-2.5 right-2.5 h-2 w-2 rounded-full bg-destructive ring-2 ring-background"></span>
          )}
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-[360px] rounded-[36px] p-6 bg-card border-border shadow-[0_20px_60px_rgba(0,0,0,0.08)] mt-4">
        <DropdownMenuLabel className="px-2 py-3 text-base font-bold tracking-tight text-foreground flex items-center justify-between">
            Notifications
            {unreadCount > 0 && (
        ...
        {notifications.length === 0 ? (
            <div className="py-12 flex flex-col items-center justify-center text-center text-muted-foreground/60">
                <HugeiconsIcon icon={Notification01Icon} className="w-10 h-10 mb-3 opacity-20" strokeWidth={1.5} />
            ...
            <div className="flex flex-col gap-1 mt-2">
                {notifications.map((n) => (
                    <DropdownMenuItem 
                        key={n.id} 
                        className={cn(
                            "cursor-pointer rounded-2xl p-4 focus:bg-secondary hover:bg-secondary transition-all outline-none border border-transparent hover:border-border group",
