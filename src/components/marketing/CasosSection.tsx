    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
     {featuredProblems.map((problem, index) => {
      ...
      return (
       ...
       <Link
        href={problem.href}
        className="group bg-card rounded-[36px] p-8 border border-border/50 hover:border-border hover:shadow-xl transition-all duration-500 block relative overflow-hidden"
       >
        <div className={`w-14 h-14 rounded-2xl bg-primary/10 text-primary flex items-center justify-center mb-6 group-hover:scale-110 group-hover:bg-primary group-hover:text-white transition-all duration-300`}>
         <HugeiconsIcon icon={problem.icon as any} size={28} strokeWidth={2.75} />
        </div>
        ...
        <div className="flex items-center text-primary font-medium">
         ...
         <HugeiconsIcon icon={ArrowRight01Icon} size={18} strokeWidth={2.5} className="ml-1 group-hover:translate-x-1 transition-transform duration-200" />
        </div>
       </Link>
       </AnimateIn>
      );
