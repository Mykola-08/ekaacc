...
export default function FooterPillMenu() {
  ...
  const items = [
    ...
    { href: '/reservar', label: t('nav.bookNow'), icon: Calendar },
  ];
        ...
        {items.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                "flex flex-col items-center justify-center w-auto min-w-14 flex-1 py-1 rounded-full transition-all duration-300 relative group",
            ...
            >
              <div className={cn(
                ...
                isActive ? "bg-amber-100/60" : "bg-transparent group-hover:bg-gray-100"
              )}>
                <item.icon className={cn("w-4 h-4 sm:w-5 sm:h-5", isActive && "fill-current")} />
              </div>
              
              <span className="text-[9px] sm:text-[10px] mt-0.5 text-center leading-tight whitespace-nowrap overflow-hidden text-ellipsis max-w-16 sm:max-w-none">{item.label}</span>
