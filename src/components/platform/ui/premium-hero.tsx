...
const premiumHeroVariants = cva('relative overflow-hidden bg-linear-to-br', {
  variants: {
    variant: {
...
const premiumHeroContentVariants = cva('relative z-10 max-w-4xl mx-auto', {
  variants: {
    variant: {
...
export interface PremiumHeroProps
  extends React.HTMLAttributes<HTMLDivElement>, VariantProps<typeof premiumHeroVariants> {
  title?: string;
  subtitle?: string;
  description?: string;
...
const PremiumHero = React.forwardRef<HTMLDivElement, PremiumHeroProps>(
  (
    ...
    const renderFloatingElements = () => {
      ...
      return (
        <>
          <div className="bg-primary-400 animate-float absolute top-1/4 left-1/4 h-2 w-2 rounded-full" />
          <div className="bg-secondary-400 animate-float-delayed absolute top-1/3 right-1/3 h-3 w-3 rounded-full" />
          <div className="bg-accent-400 animate-float absolute bottom-1/4 left-1/3 h-1 w-1 rounded-full" />
          <div className="bg-primary-300 animate-float-delayed absolute right-1/4 bottom-1/3 h-2 w-2 rounded-full" />
        </>
      );
    };
      ...
      return (
        <div className="absolute inset-0 overflow-hidden">
          {[...Array(20)].map((_, i) => (
            <div
              key={i}
              className="bg-card/20 animate-particle absolute h-1 w-1 rounded-full"
              style={{
    ...
    return (
      <section
        ref={ref}
        className={cn(premiumHeroVariants({ variant, size, layout, rounded }), className)}
        {...props}
      >
        {/* Background Media */}
        {backgroundImage && (
            ...
            {showMediaControls && (
              <div className="absolute right-4 bottom-4 flex gap-2">
                <button
                  ...
                  className="bg-card/20 hover:bg-card/30 rounded-full p-2 text-white backdrop-blur-md transition-colors"
                >
        ...
        {scrollProgress > 0 && (
          <div className="bg-card/10 absolute top-0 left-0 h-1 w-full">
            <div
              className="from-primary-500 to-secondary-500 h-full bg-linear-to-r transition-all duration-150"
        ...
        <div className={cn(premiumHeroContentVariants({ variant }))}>
          <div className="space-y-6">
            {/* Subtitle */}
            {subtitle && (
              <p className="text-primary-600 animate-fade-in-up mb-4 text-lg font-medium md:text-xl">
                {subtitle}
              </p>
            )}
            ...
            <h1
              className={cn(
                premiumHeroTitleVariants({
                  gradient: variant === 'dark' || variant === 'premium' ? 'white' : 'primary',
                }),
                'animate-fade-in-up'
              )}
            >
            ...
            {description && (
              <p
                className={cn(
                  'animate-fade-in-up mx-auto max-w-2xl text-lg leading-relaxed text-neutral-600 md:text-xl',
                  (variant === 'dark' || variant === 'premium' || variant === 'gradient') &&
                    'text-white/80'
                )}
              >
                {description}
            ...
            {features.length > 0 && (
              <div className="animate-fade-in-up flex flex-wrap justify-center gap-4">
                {features.map((feature, index) => (
                  <div
                    key={index}
                    className="bg-card/10 flex items-center gap-2 rounded-full border border-white/20 px-4 py-2 text-sm backdrop-blur-md"
                  >
              ...
              <div className="animate-fade-in-up flex flex-wrap justify-center gap-4">
                {primaryAction && (
                  <button
                    ...
                    className={cn(
                      'inline-flex transform items-center gap-2 rounded-full px-8 py-4 text-lg font-semibold transition-all duration-300',
                      'from-primary-600 to-primary-700 bg-linear-to-r text-white shadow-lg hover:shadow-xl',
                      'focus:ring-primary-500/30 focus:ring-4 focus:outline-none',
                  ...
                  >
                    ...
                    {primaryAction.icon || <ArrowRight className="h-5 w-5" />}
                  </button>
                )}
                ...
                {secondaryAction && (
                  <button
                    ...
                    className={cn(
                      'inline-flex transform items-center gap-2 rounded-full px-8 py-4 text-lg font-semibold transition-all duration-300',
                      'hover:bg-card/50 border-2 border-neutral-300 text-neutral-700 hover:border-neutral-400',
                      'focus:ring-4 focus:ring-neutral-500/20 focus:outline-none',
                      (secondaryAction.disabled || secondaryAction.loading) &&
                        'cursor-not-allowed opacity-50',
                      (variant === 'dark' || variant === 'premium' || variant === 'gradient') &&
                        'hover:bg-card/10 border-white/30 text-white'
                    )}
                  >
            ...
            {/* Stats */}
            {stats.length > 0 && (
              <div className="animate-fade-in-up grid grid-cols-2 gap-8 border-t border-white/20 pt-8 md:grid-cols-4">
            ...
            {trustIndicators.length > 0 && (
              <div className="animate-fade-in-up flex flex-wrap items-center justify-center gap-6 pt-8">
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 animate-bounce">
          <div className="flex h-10 w-6 justify-center rounded-full border-2 border-white/30">
            <div className="bg-card/60 animate-scroll-indicator mt-2 h-3 w-1 rounded-full" />
          </div>
        </div>
      </section>
    );
