...
const roles = ['Patient', 'Therapist', 'Admin'] as const;
...
export function RoleChanger() {
  ...
  const [selectedRole, setSelectedRole] = useState<Role>(
    currentUser?.role && roles.includes(currentUser.role as unknown as Role)
      ? (currentUser.role as unknown as Role)
      : 'Patient'
  );
  ...
  const updateUser = async (userData: Partial<User>) => {
    if (dataService && currentUser) {
      ...
      try {
        /* no refresh available in supabase auth context */
      } catch {}
    }
  };
  ...
  const handleChange = (role: Role) => {
    ...
    updateUser({ role });
    try {
      localStorage.setItem('eka_persona', role);
    } catch {}
    // notify other components (sidebar) to update
    try {
      window.dispatchEvent(new CustomEvent('eka_persona_change', { detail: role }));
    } catch {}
  };
