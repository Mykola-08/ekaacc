        ...
        >
          ...
          {isLoading ? (
            ...
            <CommandPrimitive.Input
              className={cn(
                'border-input placeholder:text-muted-foreground flex h-9 w-full min-w-0 bg-transparent px-3 py-1 text-base transition-[color,box-shadow] outline-none md:text-sm',
...
export function AILoadingBar() {
  ...
  if (
    ...
    return (
      <div
        className={cn(
          'border-border bg-muted text-muted-foreground absolute bottom-4 left-1/2 z-20 flex -translate-x-1/2 items-center gap-3 rounded-[calc(var(--radius)*0.6)] border px-3 py-1.5 text-sm shadow-md transition-all duration-300'
        )}
      >
  ...
  if (toolName === 'comment' && status === 'ready') {
    return (
      <div
        className={cn(
          'border-border/50 bg-popover text-muted-foreground absolute bottom-4 left-1/2 z-50 flex -translate-x-1/2 flex-col items-center gap-0 rounded-[var(--radius)] border p-1 text-sm shadow-xl backdrop-blur-sm',
          'p-3'
        )}
      >
