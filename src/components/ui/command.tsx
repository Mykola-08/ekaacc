...
function Command({
  ...
  return (
    <CommandPrimitive
      data-slot="command"
      className={cn(
        "flex size-full flex-col overflow-hidden rounded-full bg-popover p-1 text-popover-foreground",
        className
      )}
      {...props}
    />
...
function CommandDialog({
  ...
  return (
    <Dialog {...props}>
      ...
      <DialogContent
        className={cn(
          "top-1/3 translate-y-0 overflow-hidden rounded-full! p-0",
          className
        )}
...
function CommandItem({
  ...
  return (
    <CommandPrimitive.Item
      ...
      className={cn(
        "group/command-item relative flex cursor-default items-center gap-2 rounded-[calc(var(--radius)*0.8)] px-3 py-2 text-sm outline-hidden select-none in-data-[slot=dialog-content]:rounded-[var(--radius)] data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 data-selected:bg-muted data-selected:text-foreground [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 data-selected:*:[svg]:text-foreground",
        className
      )}
      {...props}
    >
