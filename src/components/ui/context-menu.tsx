...
const ContextMenu = ({
  ...
  return (
    <ContextMenuContext.Provider value={{ isOpen, setIsOpen: handleOpenChange }}>
...
const ContextMenuSub = ({
  ...
  return (
    <ContextMenuSubContext.Provider value={{ isOpen, setIsOpen: handleOpenChange }}>
      <ContextMenuPrimitive.Sub open={isOpen} onOpenChange={handleOpenChange} {...props}>
        {children}
    ...
    <AnimatePresence>
      {context?.isOpen && (
        <ContextMenuPrimitive.SubContent
          ...
          asChild
          className={cn(
            'bg-popover text-popover-foreground z-50 min-w-[8rem] origin-[--radix-context-menu-content-transform-origin] overflow-hidden rounded-lg ring-foreground/10 ring-1 p-1 shadow-md',
            className
          )}
          {...props}
        >
      ...
      <AnimatePresence>
        {context?.isOpen && (
          <ContextMenuPrimitive.Content
            ...
            asChild
            className={cn(
              'bg-popover text-popover-foreground z-50 max-h-[--radix-context-menu-content-available-height] min-w-[8rem] origin-[--radix-context-menu-content-transform-origin] overflow-x-hidden overflow-y-auto rounded-lg ring-foreground/10 ring-1 p-1 shadow-md',
              className
            )}
            {...props}
          >
...
>(({ className, children, ...props }, ref) => (
  ...
  >
    <span className="absolute left-2 flex h-3.5 w-3.5 items-center justify-center">
      <ContextMenuPrimitive.ItemIndicator>
        <HugeiconsIcon icon={CircleIcon} className="h-2 w-2" />
      </ContextMenuPrimitive.ItemIndicator>
    </span>
    {children}
  </ContextMenuPrimitive.RadioItem>
