...
const Tool = ({ toolPart, defaultOpen = false, className }: ToolProps) => {
  ...
  const getStateBadge = () => {
    ...
    switch (state) {
      case 'input-streaming':
        return <span className={cn(baseClasses, 'bg-blue-100 text-blue-700')}>Processing</span>;
      case 'input-available':
        return <span className={cn(baseClasses, 'bg-orange-100 text-orange-700')}>Ready</span>;
      case 'output-available':
        return <span className={cn(baseClasses, 'bg-green-100 text-green-700')}>Completed</span>;
      case 'output-error':
        return <span className={cn(baseClasses, 'bg-red-100 text-red-700')}>Error</span>;
      default:
        return <span className={cn(baseClasses, 'bg-gray-100 text-gray-700')}>Pending</span>;
    }
  };
  ...
  return (
    <div
      className={cn(
        'border-border mt-3 overflow-hidden rounded-[calc(var(--radius)*0.8)] border',
        className
      )}
    >
          ...
          <div className="bg-background space-y-3 p-3">
            ...
            {state === 'output-error' && toolPart.errorText && (
              <div>
                ...
                <div className="bg-background rounded border border-red-200 p-2 text-sm">
                  {toolPart.errorText}
                </div>
              </div>
            )}
