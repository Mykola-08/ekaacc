...
interface AuthContextType extends AuthState {
  ...
  hasPermission: (
    permission:
      | { group: PermissionGroup; action: PermissionAction; conditions?: Record<string, any> }
      | string
  ) => boolean;
  canAccessResource: (
    resource:
      | string
      | { resource: string; action: PermissionAction; context?: Record<string, any> },
    action?: string,
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const {
      data: { subscription },
    } = (supabase.auth as any).onAuthStateChange(async (event, session) => {
      if (!mounted) return;
  ...
  const updateProfile = async (updates: Partial<UserProfile>) => {
    ...
    try {
      const { error } = await safeSupabaseUpdate<any>('user_profiles', updates, {
        id: state.user.id,
      });
  ...
  const updatePreferences = async (updates: Partial<UserPreference>) => {
    ...
    try {
      const { error } = await safeSupabaseUpdate<any>('user_preferences', updates, {
