...
import {
  ...
  UserInteraction,
} from '@/lib/platform/services/behavioral-tracking-service';
...
export const useBehavioralTracking = (options: UseBehavioralTrackingOptions = {}) => {
  ...
  const trackPageView = useCallback(
    (pagePath?: string, metadata?: Record<string, any>) => {
      if (!user) return;
  ...
  const trackMoodLog = useCallback(
    (mood: string, intensity?: number, metadata?: Record<string, any>) => {
      if (!user) return;
  ...
  const trackExerciseCompletion = useCallback(
    (exerciseType: string, duration?: number, metadata?: Record<string, any>) => {
      if (!user) return;
  ...
  const trackGoalAchievement = useCallback(
    (goalId: string, goalTitle: string, metadata?: Record<string, any>) => {
      if (!user) return;
  ...
  const trackCrisisInteraction = useCallback(
    (crisisType: string, severity: 'low' | 'medium' | 'high', metadata?: Record<string, any>) => {
      if (!user) return;
  ...
  const trackAppointmentBooked = useCallback(
    (appointmentId: string, therapistId: string, metadata?: Record<string, any>) => {
      if (!user) return;
  ...
  const trackMessageSent = useCallback(
    (recipientId: string, messageType: string, metadata?: Record<string, any>) => {
      if (!user) return;
