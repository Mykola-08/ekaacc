...
import { Database } from '@/types/marketing/supabase';
...
const createSupabaseClient = (): SupabaseClient<Database> => {
  ...
  return new Proxy({} as any, {
    get: (_, prop) => {
        ...
        return () => {
          const createChain = () => {
            const chainStub = {
              ...
              limit: createChain,
              single: () =>
                Promise.resolve({ data: null, error: { message: 'Missing Supabase credentials' } }),
              maybeSingle: () => Promise.resolve({ data: null, error: null }),
              then: (resolve: any) =>
                Promise.resolve({
                  data: [],
                  error: { message: 'Missing Supabase credentials' },
                }).then(resolve),
            };
            return chainStub;
          };
      ...
      console.warn(
        `Attempting to access supabase.${String(prop)} with missing environment variables`
      );
      return undefined;
    },
