...
import { supabaseAdmin } from '@/lib/platform/supabase/admin';
...
if (bot) {
  ...
  bot.start((ctx) => {
    ctx.reply(
      'Welcome to EKA Booking & Account Bot!',
  ...
  bot.command('book', (ctx) => {
    ctx.reply(
      ...
      Markup.inlineKeyboard([[Markup.button.webApp('Book Now', `${webAppUrl}/telegram/book`)]])
    );
  });
  ...
  bot.command('june', (ctx) => {
    ctx.reply(
      'June "Rastanovki" Program. Choose an option:',
      Markup.inlineKeyboard([
  ...
  bot.action('june_info', (ctx) => {
    ctx.reply(
      'June "Rastanovki" Program details...\n\nChoose an option:',
      Markup.inlineKeyboard([
        [Markup.button.callback('Group Session', 'june_group')],
        [Markup.button.callback('Individual Session', 'june_individual')],
      ])
  ...
  bot.action('june_group', (ctx) => {
    ctx.reply(
      'To register for the June Group Session, please open the registration form:',
      Markup.inlineKeyboard([
  ...
  bot.action('june_individual', (ctx) => {
    ctx.reply(
      'To register for an Individual Session, please open the registration form:',
      Markup.inlineKeyboard([
