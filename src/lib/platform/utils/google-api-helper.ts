...
export async function fetchGoogleAPI(
  ...
  if (!accessToken) {
    throw new Error(
      'No valid Google access token available. Please reconnect your Google account.'
    );
  }
...
export async function getGoogleProfile(userId: string) {
  const response = await fetchGoogleAPI(userId, 'https://www.googleapis.com/oauth2/v2/userinfo');
